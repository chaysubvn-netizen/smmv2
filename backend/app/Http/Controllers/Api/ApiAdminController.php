...
class ApiAdminController extends Controller
{
    ...
    public function dashboard(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'periods' => $periodStats,
                'daily' => $daily,
                'revenue_table' => array_reverse(array_slice($daily, -7)),
    ...
    public function authLogs(Request $request): JsonResponse
    {
        ...
        $query = \App\Models\AuthActivityLog::query()
            ->where('domain', $this->domain($request));
        ...
        return response()->json([
            ...
            'data' => $page,
            'meta' => [
                'last_24h' => \App\Models\AuthActivityLog::where('domain', $domain)->where('created_at', '>=', $dayAgo)->count(),
                'successful_24h' => \App\Models\AuthActivityLog::where('domain', $domain)->where('event', 'login_success')->where('created_at', '>=', $dayAgo)->count(),
                'failed_24h' => \App\Models\AuthActivityLog::where('domain', $domain)->whereIn('event', ['login_failed', 'login_blocked', 'two_factor_failed'])->where('created_at', '>=', $dayAgo)->count(),
                'unique_ips_24h' => \App\Models\AuthActivityLog::where('domain', $domain)->where('created_at', '>=', $dayAgo)->distinct('ip_address')->count('ip_address'),
            ],
        ]);
    }
    ...
    public function uploadSettingImage(Request $request): JsonResponse
    {
        ...
        if ($uploadedFile && !$uploadedFile->isValid()) {
            $uploadLimit = min(
                $this->phpSizeInBytes((string) ini_get('upload_max_filesize')),
                $this->phpSizeInBytes((string) ini_get('post_max_size'))
            );
            $uploadError = match ($uploadedFile->getError()) {
                UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE => 'File vượt giới hạn ' . $this->formatBytes($uploadLimit) . ' của máy chủ.',
                UPLOAD_ERR_PARTIAL => 'File chỉ được tải lên một phần. Vui lòng kiểm tra kết nối và thử lại.',
                UPLOAD_ERR_NO_TMP_DIR => 'Máy chủ chưa cấu hình thư mục tạm để nhận file upload.',
                UPLOAD_ERR_CANT_WRITE => 'Máy chủ không thể ghi file vào ổ đĩa. Vui lòng kiểm tra quyền thư mục tạm.',
                UPLOAD_ERR_EXTENSION => 'Một PHP extension trên máy chủ đã chặn file upload.',
                default => 'Máy chủ không nhận được file upload hợp lệ.',
            };
    ...
    private function phpSizeInBytes(string $value): int
    {
        $value = trim($value);
        $bytes = (int) $value;
    ...
    public function destroyAllRateUpdates(Request $request): JsonResponse
    {
        $this->authorizeAdmin($request);
        $model = $this->resourceModel('rate-updates');
        $query = $model->newQuery();
        $this->applyResourceDomain($query, $model, 'rate-updates', $request);
        $deleted = $query->delete();

        return response()->json([
            'status' => true,
            'message' => 'Đã xóa tất cả lịch sử cập nhật giá.',
            'deleted' => $deleted,
        ]);
    }
    ...
    public function telegramConfig(Request $request): JsonResponse
    {
        ...
        return response()->json(['status' => true, 'data' => [
            ...
            'telegram_link' => $config->tele_bot_username ? 'https://t.me/' . $config->tele_bot_username : '',
    ...
    public function registerTelegramWebhook(Request $request): JsonResponse
    {
        ...
        $config->fill(['tele_bot_username' => $username, 'telegram_webhook_url' => $data['telegram_webhook_url'], 'telegram_status' => 'active'])->save();
    ...
    public function updateUser(Request $request, int $id): JsonResponse
    {
        ...
        if ($passwordChanged) $user->password = Hash::make($data['new_password']);
        $user->save();
        if ($passwordChanged) $user->tokens()->delete();
