...
class ApiClientController extends Controller
{
    ...
    private function paymentConfig(Request $request): ?Config
    {
        ...
        $domain = str_starts_with($host, 'api.') ? substr($host, 4) : $host;

        // Keep payment settings in sync with the admin settings page. An account
        // may retain an old domain or alias, so the current request host must win.
        return Config::where('domain', $domain)->first()
            ?? Config::where('domain', $host)->first()
            ?? Config::where('domain', $user->domain)->first()
            ?? Config::first();
    }
    ...
    public function changePassword(Request $request)
    {
        ...
        $user->save();
        AuthActivityLogger::record($request, 'password_changed', $user, true, 'Đã đổi mật khẩu và thu hồi toàn bộ phiên đăng nhập.');
        $user->tokens()->delete();
