      ...
      <Card loading={loading} className={styles.card} title={<Space><RobotOutlined />Cấu hình Telegram Bot</Space>}>
        <Form form={form} layout="vertical" requiredMark={false} initialValues={{ telegram_status: 'inactive' }}>
          <div className={styles.grid}>
            <Form.Item name="tele_bot_username" label="Bot Username"><Space.Compact block><Button className={styles.prefix} disabled>@</Button><Input readOnly disabled placeholder="dhsolution_bot" /></Space.Compact></Form.Item>
            <Form.Item name="telegram_link" label="Link Telegram Bot" rules={[{ type: 'url', message: 'Link Telegram không hợp lệ.' }]} tooltip="Link bot được tạo tự động dựa trên Bot Username sau khi đăng ký Webhook."><Input prefix={<LinkOutlined />} readOnly disabled placeholder="https://t.me/dhsolution_bot" /></Form.Item>
